...
// Prove creates proof from a circuit
// If a constraint is not solved, the error is not caught when the circuit is solved. 
// If the constraint system is not satisfied, the error will be caught after the verification part.
// The function is here for benchmark purpose (where one just wants to benchmakr the prover, whether
// or not the circuit is satisfied).
func ProveUnsafe(r1cs *{{toLower .Curve}}backend.R1CS, pk *ProvingKey, solution map[string]interface{}) (*Proof, error) {
	nbPrivateWires := r1cs.NbWires-r1cs.NbPublicWires
